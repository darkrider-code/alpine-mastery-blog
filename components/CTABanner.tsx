...
export default function CTABanner({ title, description, showBrand = true, product }: CTABannerProps) {
  ...
  return (
    <section className="mt-16 p-8 sm:p-10 rounded-2xl bg-gradient-to-br from-bg-secondary to-bg-card border border-accent/30 text-center">
      {showBrand && (
      ...
      <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
        <a href={urls.freeAnalysis} className="inline-flex items-center justify-center rounded-xl bg-accent px-8 py-4 text-base font-bold text-bg-primary transition hover:bg-accent-hover">
          Testa gratis analys
        </a>
        <a href={urls.createAccount} className="inline-flex items-center justify-center rounded-xl border border-accent px-8 py-4 text-base font-semibold text-accent transition hover:bg-accent/10">
          Skapa konto
        </a>
      </div>
    </section>
